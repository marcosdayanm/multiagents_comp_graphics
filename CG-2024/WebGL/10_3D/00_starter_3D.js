...
function main() {
  ...
  objects.model.bufferInfo = twgl.createBufferInfoFromArrays(
    gl,
    objects.model.arrays
  );
  objects.model.vao = twgl.createVAOFromBufferInfo(
    gl,
    programInfo,
    objects.model.bufferInfo
  );
...
function drawScene() {
  ...
  let scale = [
    objects.model.transforms.s.x,
    objects.model.transforms.s.y,
    objects.model.transforms.s.z,
  ];

  let rotationXRad = objects.model.transforms.rr.x;
  let rotationYRad = objects.model.transforms.rr.y;
  let rotationZRad = objects.model.transforms.rr.z;

  // Create the individual transform matrices
  const scaMat = m4.scale(scale);
  const traMat = m4.translation(translation);
  const rotMatX = m4.rotationX(rotationXRad);
  const rotMatY = m4.rotationY(rotationYRad);
  const rotMatZ = m4.rotationZ(rotationZRad);
...
function setupUI() {
  ...
  rotFolder.add(objects.model.transforms.rd, "x", 0, 360).onChange((value) => {
    objects.model.transforms.rd.x = value;
    objects.model.transforms.rr.x = (value * Math.PI) / 180;
  });
  rotFolder.add(objects.model.transforms.rd, "y", 0, 360).onChange((value) => {
    objects.model.transforms.rd.y = value;
    objects.model.transforms.rr.y = (value * Math.PI) / 180;
  });
  rotFolder.add(objects.model.transforms.rd, "z", 0, 360).onChange((value) => {
